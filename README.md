...
**For reviewers / research contact:** [3분 리뷰어 가이드](docs/PORTFOLIO_WALKTHROUGH.md) · [프로젝트 상태](docs/PROJECT_STATUS.md) · [교수 컨택용 연구 브리프](docs/professor_contact_brief.md) · [모델 카드](docs/MODEL_CARD.md) · [매일 개발 스프린트](docs/daily_sprint_plan.md)

<br>

### 연구 컨택용 핵심 요약

이 프로젝트는 단순 웹 대시보드가 아니라, **다채널 센서 데이터 기반 PHM/예지보전 연구 포트폴리오**입니다.  
가공 설비·AMR·제조 로봇에서 발생하는 진동, 배터리, 온도, 동작상태, 누적거리 같은 운전 데이터를 이용해 **상태진단, 고장예측, 데이터 한계 분석, 디지털 트윈 관제**까지 하나의 파이프라인으로 구현했습니다.

교수님 컨택 시 강조할 연구 연결점:

- **PHM / Fault Diagnosis:** 정상-고장 상태 분류, 희귀 고장 미탐 문제 분석
- **Sensor Feature Engineering:** FFT 주파수 피처, drift 피처, 누적 운전 피처 복원
- **Industrial Generalization:** 처음 보는 로봇 validation split으로 일반화 성능 검증
- **Digital Twin:** 실시간 진단 결과를 3D 관제 화면과 작업지시로 연결
- **Manufacturing Extension:** CNC/로봇 가공 공정의 진동·전류·비전 데이터 기반 품질예측으로 확장 가능
...
| **리뷰어 가이드** | 3분 시연 순서·직무별 증거·면접 포인트 ([문서](docs/PORTFOLIO_WALKTHROUGH.md), `/api/reviewer-brief`) |
