...
3. Open the operations APIs.
   Use `/api/work-orders`, `/api/reliability`, `/api/trend`, and `/api/history` to show maintenance actions, MTBF/MTTR, fleet trends, and stored telemetry.
...
| Robotics / Smart Factory Engineer | AGV/FAB digital twin, fleet monitoring, fault triage, maintenance action loop | `/twin`, `/ws`, `/api/snapshot`, `/api/work-orders`, `src/fab_layout.py` |
...
| 1:30-2:05 | "The backend turns warnings into maintenance work orders, stores events, and exposes trend/reliability analytics." |
