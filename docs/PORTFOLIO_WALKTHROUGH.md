...
3. Open the operations APIs.
   Use `/api/reliability`, `/api/trend`, and `/api/history` to show MTBF/MTTR, fleet trends, and stored telemetry.
...
|---|---|---|
| Data / AI Engineer | Feature engineering, honest validation, serving, MLOps, data quality | `/predict`, `/api/data-quality`, `/api/drift`, `/api/model-card` |
| Robotics / Smart Factory Engineer | AGV/FAB digital twin, fleet monitoring, fault triage | `/twin`, `/ws`, `/api/snapshot`, `src/fab_layout.py` |
...
| Semiconductor Equipment / Operations | Health index, MTBF/MTTR, availability, maintenance priority | `/api/reliability`, `/api/history`, `/api/trend`, `/twin` |
...
| 1:30-2:05 | "The backend stores warning and low-health events, then exposes trend and reliability analytics." |
...
Portfolio demo: **88% complete**.

The core experience is demo-ready. The remaining highest-impact work is recording
a polished 2-3 minute demo video and adding a lightweight MQTT-style ingestion
split to make the edge-cloud architecture more explicit.
